...
work_docker() {
   ...
   docker-compose -f docker-compose_build.yml build terratools-jenkins

   dockerimage="terratools-jenkins:${TERRAFORM_VERSION}"
   docker tag "${dockerimage}" "$docker_owner/terratools-jenkins:${dockerhubtag}"
   test -f "$docker_credentials" && docker push "$docker_owner/terratools-jenkins:${dockerhubtag}"
}
